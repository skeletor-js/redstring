...
7. [Similarity Endpoints](#similarity-endpoints)
...
- **Case Similarity**: Find similar cases using multi-factor weighted scoring
...
### POST /api/clusters/preflight

Check dataset size and get tier classification before running cluster analysis.

This endpoint should be called before `/api/clusters/analyze` to determine if the dataset requires filtering or user confirmation.

**Request Body:** Same as `/api/clusters/analyze` (ClusterAnalysisRequest)
...
```json
{
  ...
  "message": "Analysis will take approximately 45 seconds",
  "suggestions": []
}
```
...
|------|------------|----------|
| 1 | < 10,000 | Instant analysis, no warning needed |
| 2 | 10,000 - 50,000 | Shows time estimate, user confirms to proceed |
| 3 | > 50,000 | Requires additional filtering before analysis |

**Response Fields:**

| Field | Type | Description |
|-------|------|-------------|
| `case_count` | integer | Number of cases matching the filter |
| `tier` | integer | Dataset tier (1, 2, or 3) |
| `tier_name` | string | Tier name: "small", "medium", or "large" |
| `can_proceed` | boolean | Whether analysis can proceed without additional filtering |
| `estimated_time_seconds` | float | Estimated analysis time (Tier 2 only) |
| `message` | string | Human-readable status message |
| `suggestions` | array | Filter suggestions for Tier 3 datasets |

**Example:**
```bash
curl -X POST http://localhost:5000/api/clusters/preflight \
...
### GET /api/similarity/find/{case_id}

Find cases similar to the specified case using weighted multi-factor similarity scoring.

This endpoint compares a reference case against all other cases with the same victim sex, returning the most similar cases with detailed factor breakdowns.

**Path Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `case_id` | string | Case ID to find similar cases for (database ID) |

**Query Parameters:**

| Parameter | Type | Default | Range | Description |
|-----------|------|---------|-------|-------------|
| `limit` | integer | 50 | 1-100 | Maximum number of similar cases to return |
| `min_score` | float | 30.0 | 0-100 | Minimum similarity score threshold |

**Similarity Factors (weights):**

| Factor | Weight | Description |
|--------|--------|-------------|
| Weapon | 30% | Exact match (100) or same category (70) |
| Geographic | 25% | Distance-based scoring within 100 miles |
| Victim Age | 20% | Age difference scoring (100 if within 10 years) |
| Temporal | 15% | Year difference scoring (100 if within 5 years) |
...
| Field | Type | Description |
|-------|------|-------------|
| `reference_case_id` | string | The case being compared against |
| `similar_cases` | array | List of similar cases with scores and factors |
| `total_found` | integer | Total number of similar cases found |
...
| `similarity_score` | float | Overall similarity score (0-100) |
| `matching_factors` | object | Individual factor scores (0-100 each) |
| `year` | integer | Year of the case |
| `state` | string | State where case occurred |
| `weapon` | string | Weapon used |
| `vic_age` | integer | Victim age |
| `vic_sex` | string | Victim sex |
| `vic_race` | string | Victim race |
| `solved` | integer | Solved status (0=unsolved, 1=solved) |
| `circumstance` | string | Circumstance |
| `relationship` | string | Victim-offender relationship |

**Example:**
```bash
curl "http://localhost:5000/api/similarity/find/12345?limit=20&min_score=50"
```
